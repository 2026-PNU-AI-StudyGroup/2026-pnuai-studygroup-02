...
class LLMTimeoutError(RuntimeError):
    """LLM API 호출 시간이 초과된 경우."""


class LLMResponseParseError(ValueError):
    """LLM 응답 JSON 파싱 또는 스키마 검증에 실패한 경우."""
...
8. steps의 각 단계에는 재료 분량(예: 1큰술, 200g, 1개)을 구체적인 숫자로 명시하세요.
   볶기·끓이기·굽기·찌기처럼 불에 올려 가열하는 단계에는 조리 시간
   (예: 중불에서 5분간)도 반드시 포함하세요.
   썰기·다지기·씻기처럼 불을 쓰지 않는 손질 단계에는 시간을 억지로 붙이지 말고,
   대신 크기나 두께(예: 1cm 두께로, 한입 크기로) 등 자연스러운 구체적 표현을 쓰세요.
   '적당히', '알맞게', '약간' 같은 모호한 표현은 쓰지 마세요.
...
def _call_gemini(
    ...
    try:
        ...
        raise LLMResponseParseError(
            "Gemini 응답 JSON 파싱 또는 "
            "RecipeResponse 스키마 검증에 실패했습니다."
        ) from exc
